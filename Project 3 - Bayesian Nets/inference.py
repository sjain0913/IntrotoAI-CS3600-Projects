...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # establishing the counter container datatype
        allPossible = util.Counter()

        # The following are just print statements I used in the beginning.
        # print("---------------")
        # print(noisyDistance)
        ...
        if noisyDistance is None:
            ...
            # count of jail position set to 1 in allPossible
            allPossible[self.getJailPosition()] = 1.0

        else:
            # iterating through all legal positions
            for position in self.legalPositions:

                # calculating distance
                dist = abs(position[0] - pacmanPosition[0]) + abs(position[1] - pacmanPosition[1])

                # count of position set according to values calculated
                allPossible[position] = emissionModel[dist] * self.beliefs.get(position)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for legalPosition in self.legalPositions:
            ...
            # calculation of new game state using initial game state and legal position
            gameStateNew = self.setGhostPosition(gameState, legalPosition)

            # calculation of distance using new game state
            distance = self.getPositionDistribution(gameStateNew)

            # iterating through distance tuples of their position and probability
            for position, probability in distance.items():

                # adding to count of position in allPossible using probability
                allPossible[position] += self.beliefs[legalPosition] * probability

        # changing beliefs to allPossible for the update        
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # generating new particles list
        particlesList = []

        # establishing counter to go through the required number from self.numParticles
        counter = 0

        # run loop until we have gone through all the values upto self.numParticles
        while counter < self.numParticles:

            #adding to particlesList based on positions that are legal
        	particlesList.append(self.legalPositions[counter % len(self.legalPositions)])

        	counter += 1

        # setting particles list to the new one we just generated
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            # changing particles list according to JailPosition of each particle upto numParticles
        ...
        for particle in self.particles:
            ...
            # adding to the count of particle in beliefs
        	beliefs[particle] += 1

        # updating beliefs for each individual particle
        for location, value in beliefs.items():

        	distance = util.manhattanDistance(location, pacmanPosition)

        	beliefs[location] = value * emissionModel[distance]

        # only reset particles if the sum of values from beliefs is not 0
    ...
    def elapseTime(self, gameState):
        ...
        # generating list of new particles with updated beliefs
        newParticles = []

        # iterating through every particle
        for particle in self.particles:

            # calculation of new position distance
        	newPositionDistance = self.getPositionDistribution(self.setGhostPosition(gameState, particle))

            # adding to the newParticles list using the new position distance and the helper method
        	newParticles.append(util.sample(newPositionDistance))

        # changing particles to the new particles list
    ...
    def getBeliefDistribution(self):
        ...
        # establishing the counter container datatype
        beliefDistance = util.Counter()

        # iterating through all the particles
        for particle in self.particles:

            # adding to the count of particle in beliefDistance
    		beliefDistance[particle] += 1.0

        beliefDistance.normalize()

        # returning current belief state
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # getting list of particles and randomizing it
        particles = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(particles)

        # establishing counrer so loop only runs upto number of Particles
        counter = 0
        self.particles = []
        ...
        while (counter < self.numParticles):
            ...
            # adding to self.particles
            self.particles.append(particles[counter % len(particles)])
            counter += 1
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        # updating joint beliefs based on our noisy observations, going through each particle
        for particle in self.particles:

            # initial probability of 1
        	probability = 1.0

        	for ghostItem in range(0, self.numGhosts):

                # probability updated if count of ghostItem isn't None
        		if noisyDistances[ghostItem] != None:
        			ghostPosition = particle[ghostItem]
        			distance = util.manhattanDistance(ghostPosition, pacmanPosition)
        			probability = probability * emissionModels[ghostItem][distance]
        		else:
        			particle = self.getParticleWithGhostInJail(particle, ghostItem)
        ...
        if all(value == 0 for value in beliefs.values()):
            ...
            # going through each ghost of every particle (nested for loops)
        	for i in range(len(self.particles)):
        		...
        		for j in range(self.numGhosts):
        			...
        			if noisyDistances[j] == None:
        				...
        				# checking if the ghost is in jail
        				oldParticle = self.particles[j]

                        # establishing newParticle
        ...
        # repopulating self.particles using the helper function
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            ...
            # incrementing count of each particle in beliefDistance
    		beliefDistance[particle] += 1.0
